...
| In that guide, graph = execution map | Nodes have one job, input, structured output, named failure; edges exist only if the next step reads the previous output | High | 실제 구조 |
