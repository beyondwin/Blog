...
import { reviewCoverRedistributionReceiptSchema } from './review-cover-redistribution.mjs';
...
const mediaItemSchema = z.object({
  ...
  if (Number(Boolean(item.sourceUrl)) + Number(Boolean(item.sourcePath)) !== 1) {
    context.addIssue({ code: 'custom', message: 'media item requires exactly one of sourceUrl or sourcePath', path: ['sourceUrl'] });
  }
  if (Number(item.width !== undefined) + Number(item.height !== undefined) === 1) {
    context.addIssue({ code: 'custom', message: 'media item dimensions require both width and height', path: ['width'] });
  }
  if (item.redistributionApproval && item.kind !== 'book-cover') {
    context.addIssue({ code: 'custom', message: 'redistribution approval is only valid for book-cover media', path: ['redistributionApproval'] });
  }
  const decisionBound = Boolean(item.sourcePath && generatedDecisionManifestPathSchema.safeParse(item.sourcePath).success);
  if (decisionBound && item.sourceKind !== 'repository-generated') {
    context.addIssue({ code: 'custom', message: 'decision-bound media requires sourceKind repository-generated', path: ['sourceKind'] });
  }
  if (item.sourceKind === 'repository-generated' && !item.generation) {
    context.addIssue({ code: 'custom', message: 'repository-generated sourceKind requires generation metadata', path: ['generation'] });
  }
  if (item.generation && item.sourceKind !== 'repository-generated') {
    context.addIssue({ code: 'custom', message: 'sourceKind must be repository-generated when generation metadata is present', path: ['sourceKind'] });
  }
  if (item.generation && (!item.sourcePath || !generatedDecisionManifestPathSchema.safeParse(item.sourcePath).success)) {
    context.addIssue({ code: 'custom', message: 'generated media requires its durable decision manifest as sourcePath', path: ['sourcePath'] });
  }
  if (item.generation && item.sourceUrl) {
    context.addIssue({ code: 'custom', message: 'generated media must not use sourceUrl', path: ['sourceUrl'] });
  }
});

const manifestSchema = z.object({ version: z.literal(1), items: z.array(mediaItemSchema) }).strict();
