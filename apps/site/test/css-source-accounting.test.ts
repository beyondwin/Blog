...
describe('route-scoped critical CSS source accounting', () => {
  it('emits shared tokens and shell exactly once with only the matching route styles', async () => {
    const [tokens, shell, scene, reading, article, review, memory] = await Promise.all([
    ...
    const sources = { tokens, shell, scene, reading, article, review, memory };
    ...
    expect(articleDetail).toContain('.article-masthead');
    expect(articleDetail).not.toContain('.book-sheet');
    expect(reviewDetail).toContain('.book-sheet');
    expect(reviewDetail).not.toContain('.memory-thought');
    expect(memoryDetail).toContain('.memory-thought');
    expect(memoryDetail).not.toContain('.article-masthead');
  });
});
