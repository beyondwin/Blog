...
describe('PostgresHybridRetriever', () => {
  ...
  it('embeds normalized query once, scopes both branches to immutable binding/release, and resolves catalog evidence', async () => {
    ...
    expect(query.mock.calls[0]![0]).toContain('b.binding_id=$1::uuid');expect(query.mock.calls[0]![0]).not.toContain("b.state='active'");expect(query.mock.calls[0]![0]).toContain('c.answer_release_id=$2');expect(query.mock.calls[0]![0]).toContain('LIMIT 20');
    expect(query.mock.calls[1]![0].replace(/\s+/gu,' ')).toContain('1-(c.embedding <=> $2::vector)');expect(query.mock.calls[1]![0]).not.toContain("b.state='active'");expect(query.mock.calls[1]![0]).toContain('c.answer_release_id=$3');expect(query.mock.calls[1]![0]).toContain('ORDER BY c.embedding <=> $2::vector,c.chunk_id ASC LIMIT 20');
