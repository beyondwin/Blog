...
describe('real Postgres hybrid retrieval isolation',()=>{
  it('continues an admitted snapshot after retirement while isolating binding, release, and vector ties',async()=>{
    ...
    await pool.query("UPDATE public_answer_release_bindings SET state='retired' WHERE binding_id=$1",[bindingId]);const retired=await retriever.retrieve({question:'public text',catalog,limit:6,signal:new AbortController().signal});expect(retired.evidence.map((item)=>item.evidenceId)).toEqual(['1'.repeat(64),'2'.repeat(64),'3'.repeat(64)]);expect(embedCalls).toBe(2);
  });
});
